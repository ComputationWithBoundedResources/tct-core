...
defaultTctConfig
  :: ProofData i => (FilePath -> IO (Either String i)) -> TctConfig i
...
mkParser
  :: [StrategyDeclaration i o]
  -> String
  -> O.Parser m
  -> O.ParserInfo (TctAction m)
mkParser ps vers mparser = O.info
  (versioned <*> listed <*> O.helper <*> interactive <|> tctp)
  desc
 where
  listed = O.infoOption (PP.display . PP.vcat $ map PP.pretty ps)
    $ mconcat [O.long "list", O.help "Display list of strategies."]
  versioned = O.infoOption vers $ mconcat
    [O.long "version", O.short 'v', O.help "Display Version.", O.hidden]
  ...
  tctp =
    fmap Run
      ...
      <$> O.optional
            (O.option
              (O.str >>= readAnswerFormat)
              (mconcat
                [ O.short 'a'
                , O.long "answer"
                , O.helpDoc . Just $ PP.vcat
                  [ PP.hsep
                    [ PP.text (writeAnswerFormat SilentAnswerFormat)
                    , PP.text "- silent"
                    ]
                  , PP.hsep
      ...
      <*> O.optional
            (O.option
              (O.str >>= readProofFormat)
              (mconcat
                [ O.short 'p'
                , O.long "proof"
                , O.helpDoc . Just $ PP.vcat
                  [ PP.hsep
                    [ PP.text (writeProofFormat SilentProofFormat)
                    , PP.text "- silent"
                    ]
                  , PP.hsep
                    [ PP.text (writeProofFormat DefaultProofFormat)
                    , PP.text "- default proof"
                    ]
                  ]
                ]
              )
...
run' m k = do
  ...
  let state tmp = TctROState { startTime     = time
                             , stopTime      = Nothing
                             , tempDirectory = tmp
                             , kvPairs       = M.empty
                             }
...
runTctWithOptions
  :: (ProofData i, Declared i i)
  => (TctConfig i -> opt -> TctConfig i)
  -> Options opt
  -> TctConfig i
  -> IO ()
...
runTctWithOptions' theStrategies theUpdate theOptions cfg = do
  r <- runErroneousIO $ do
    action <- liftIO $ O.execParser $ mkParser theStrategies
                                               (version cfg)
                                               theOptions
    case action of
      ...
      Run opts       -> do
        let
          TctOptions { strategyName_ = theStrategyName, timeout_ = theTimeout, problemFile_ = theProblemFile }
            = opts
          ucfg = updateTctConfig theUpdate opts cfg
        let
          TctConfig { parseProblem = theParser, putAnswer = theAnswer, putProof = theProof, defaultStrategy = theDefaultStrategy }
            = ucfg
        ...
        st <- maybe (return theDefaultStrategy)
                    (liftEither . parseStrategy)
                    theStrategyName
  ...
  case r of
    Left err ->
      PP.putPretty (PP.text "ERROR")
        >> PP.hPutDoc stderr (PP.pretty err PP.</> PP.text "")
        >> exitFailure
    Right _ -> exitSuccess
 where
  ...
  ppAnswer (Right f) = f
  ppProof (Left f@DefaultProofFormat) a =
    \pt -> putProofFormat f pt >> ppAnswer a pt
