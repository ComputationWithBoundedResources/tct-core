...
import           Tct.Core.Data.Answer           ( prettyDefaultFormatSep
                                                , termcomp
                                                )
import           Tct.Core.Data.Certificate      ( Certificate
                                                , unbounded
                                                )
...
collectCertificate (Success _ certfn' subtrees) =
  certfn' (collectCertificate `fmap` subtrees)
...
ppPath :: Path -> PP.Doc
ppPath (Path i is) =
  PP.cat $ PP.punctuate PP.colon . reverse $ PP.int i : f `fmap` is
 where
...
ppNode p prob po = PP.vcat
 ...
 where
  block n e =
    PP.nest 4 (PP.text "+" PP.<+> PP.text n PP.<> PP.char ':' PP.<$$> e)
...
ppProofTree' is ppProb pt@(Open l) =
  PP.vcat [ppHeader pt is (PP.text "Open"), PP.indent 4 (ppProb l)]
ppProofTree' is _ f@(Failure r) =
  ppHeader f is (PP.text "Failure") PP.<$$> PP.indent 2 (ppReason r)


ppProofTree' path ppProb pt@(Success pn _ pts) = PP.vcat
  [ ppHeader
    pt
    path
    (     PP.text (takeWhile (`notElem` " {") (show (appliedProcessor pn)))
    PP.<> PP.text ". "
    )
 ...
 where
  ...
  ppSubTrees ls =
    PP.vcat [ ppProofTree' (split path j) ppProb t | (j, t) <- zip [1 ..] ls ]


ppHeader :: ProofTree l -> Path -> PP.Doc -> PP.Doc
ppHeader pt p s =
  PP.text (replicate (pathLength p) '*')
    PP.<+> PP.text "Step"
    PP.<+> ppPath p
    PP.<>  PP.char ':'
    PP.<+> s
  -- PP.<+> PP.group (PP.pretty (termcomp (certificate pt)))
    PP.<+> prettyDefaultFormatSep
             (\a b -> PP.text "WC:" PP.<+> a PP.<+> PP.text " --  BC:" PP.<+> b)
             (termcomp (certificate pt))


ppProofTree :: (l -> PP.Doc) -> ProofTree l -> PP.Doc
ppProofTree pp pt = ppProofTree' (Path 1 []) pp pt PP.<> if null (F.toList pt)
  then PP.empty
  else
    PP.empty
