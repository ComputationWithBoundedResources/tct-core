...
data User a = User { nmU :: Maybe Int, idU :: a} deriving Show

user :: Options () (User Int)
user = User <$>  mayoption "name" P.natural [] <*> option "id" P.natural []

parse :: String -> String
parse s = case P.parse (mkParser user) "" s of
  Left err -> show err
  Right xs -> show xs
