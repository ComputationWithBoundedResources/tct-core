...
import Tct.Core.Options (Option (..), (<#>), option, nat)
...
instance (C.Processor p) => Pretty.Pretty (TimeoutProof p) where
    ...
    pretty (NoTimeout obj) = Pretty.pretty obj
