...
closedProblem :: TrsProblem f v -> Bool
closedProblem = null . strictRules
...
options :: Options CC
options = option $ eopt `withArgLong` "complexity" `withCropped` 'c' `withDefault` RCF
...
answering = answer . returning (YES . timeUB . certificate) (const NO)
